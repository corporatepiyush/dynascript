/*
 * dyna:uring -- io_uring disk I/O (Linux only, CONFIG_IO_URING).
 *
 ...
 * so dyna:file can use the io_uring bulk path for readFile() on Linux. */
                                     ...
                                     int use_uring)
{
    ...
    if ((use_uring ? dyn_uring_read_all(path, &data, &len)
        ...
        return JS_ThrowInternalError(ctx, "dyna:uring: read failed");
    }
