/*
 * dynajs:bits -- a faithful port of Go's `math/bits` package. Self-contained,
 ...
 * leaves nothing to free. Mirrors dynajs-mathx.c's 2-tuple builders. */
                          ...
                          int argc, JSValueConst *argv)
{
    ...
    if (JS_ToUint32(ctx, &hi, argv[0]) || JS_ToUint32(ctx, &lo, argv[1]) ||
        ...
        return JS_EXCEPTION;
    if (y == 0)
        return JS_ThrowRangeError(ctx, "dynajs:bits: Div32 division by zero");
    if (y <= hi)
        return JS_ThrowRangeError(ctx, "dynajs:bits: Div32 quotient overflow");
    d = ((uint64_t)hi << 32) | lo;
                          ...
                          int argc, JSValueConst *argv)
{
    ...
    if (bits_read64(ctx, argv[0], &hi) || bits_read64(ctx, argv[1], &lo) ||
        ...
        return JS_EXCEPTION;
    if (y == 0)
        return JS_ThrowRangeError(ctx, "dynajs:bits: Div64 division by zero");
    if (y <= hi)
        return JS_ThrowRangeError(ctx, "dynajs:bits: Div64 quotient overflow");
                          ...
                          int argc, JSValueConst *argv)
{
    ...
    if (JS_ToUint32(ctx, &hi, argv[0]) || JS_ToUint32(ctx, &lo, argv[1]) ||
        ...
        return JS_EXCEPTION;
    if (y == 0)
        return JS_ThrowRangeError(ctx, "dynajs:bits: Rem32 division by zero");
    d = ((uint64_t)hi << 32) | lo;
                          ...
                          int argc, JSValueConst *argv)
{
    ...
    if (bits_read64(ctx, argv[0], &hi) || bits_read64(ctx, argv[1], &lo) ||
        ...
        return JS_EXCEPTION;
    if (y == 0)
        return JS_ThrowRangeError(ctx, "dynajs:bits: Rem64 division by zero");
    d = ((unsigned __int128)hi << 64) | lo;
...
{
    JSModuleDef *m = JS_NewCModule(ctx, "dynajs:bits", bits_init_module);
    if (!m)
        return -1;
