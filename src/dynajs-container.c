/*
 * dynajs:container -- native container data structures, self-contained and
 ...
 *
 * Ownership (same discipline as dynajs-structures.c): every container stores
 ...
 * dynajs-sort.c's sort()/binarySearch() (plain, transient functions whose
 ...
 * as dynajs-structures.c's dyn_vector_reserve. Returns 0 or -1. */
 ...
 * STORED comparator, unlike dynajs-sort.c's transient one. A NaN or 0 result
 * maps to "equal", matching Array.prototype.sort / dynajs-sort.c. */
 ...
 * resolves -- mirrors Vector.get in dynajs-structures.c exactly. */
...
{
    JSModuleDef *m = JS_NewCModule(ctx, "dynajs:container",
