/*
 * dyna:netip -- IP address + CIDR prefix parsing/manipulation, modeled on
 ...
 *            isLinkLocalMulticast } from "dyna:netip";
 *
                                 ...
                                 size_t *plen)
{
    if (argc < 1 || !JS_IsString(argv[0])) {
        JS_ThrowTypeError(ctx, "dyna:netip: argument must be a string");
        return NULL;
    }
                                    ...
                                    int argc, JSValueConst *argv)
{
    ...
    if (parse_addr(s, n, &a) != 0) {
        ...
        return JS_ThrowTypeError(ctx, "dyna:netip: invalid IP address");
    }
    JS_FreeCString(ctx, s);
                                      ...
                                      int argc, JSValueConst *argv)
{
    ...
    if (parse_prefix(s, n, &a, &bits) != 0) {
        JS_FreeCString(ctx, s);
        return JS_ThrowTypeError(ctx, "dyna:netip: invalid CIDR prefix");
    }
    JS_FreeCString(ctx, s);
                                  ...
                                  int argc, JSValueConst *argv)
{
    ...
    if (argc < 2 || !JS_IsString(argv[0]) || !JS_IsString(argv[1]))
        return JS_ThrowTypeError(ctx,
            "dyna:netip: contains(prefix, addr) expects two strings");
    ...
    if (!ok)
        return JS_ThrowTypeError(ctx,
            "dyna:netip: invalid CIDR prefix or address");
                                 ...
                                 int argc, JSValueConst *argv)
{
    ...
    if (argc < 2 || !JS_IsString(argv[0]) || !JS_IsString(argv[1]))
        return JS_ThrowTypeError(ctx,
            "dyna:netip: compareAddr(a, b) expects two strings");
...
{
    JSModuleDef *m = JS_NewCModule(ctx, "dyna:netip", dyn_netip_init_module);
    if (!m)
        return -1;
