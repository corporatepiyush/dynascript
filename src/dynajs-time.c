/*
 * dynajs:time -- time/duration utilities, self-contained, in-repo (no
 ...
 *            formatRFC3339, parseRFC3339, date, fromUnix } from "dynajs:time";
 *
                                       ...
                                       int argc, JSValueConst *argv)
{
 ...
 bad:
    JS_ThrowSyntaxError(ctx, "dynajs:time: invalid duration");
    result = JS_EXCEPTION;
                            ...
                            int argc, JSValueConst *argv)
{
    ...
    if (clock_gettime(CLOCK_REALTIME, &ts) != 0)
        return JS_ThrowInternalError(ctx, "dynajs:time: clock_gettime failed");
                                       ...
                                       int argc, JSValueConst *argv)
{
    ...
    if (nsec < 0 || nsec > 999999999)
        return JS_ThrowRangeError(ctx, "dynajs:time: nsec must be in [0, 999999999]");
                                      ...
                                      int argc, JSValueConst *argv)
{
 ...
 fail:
    JS_ThrowSyntaxError(ctx, "dynajs:time: invalid RFC3339 timestamp");
    result = JS_EXCEPTION;
...
{
    JSModuleDef *m = JS_NewCModule(ctx, "dynajs:time", dyn_time_init_module);
    if (!m)
        return -1;
