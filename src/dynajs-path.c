/*
 * dynajs:path -- POSIX filesystem-path string utilities. Self-contained,
 ...
 * dynajs:search's indexOf, which documents the opposite choice).
 *
                             ...
                             int argc, JSValueConst *argv)
{
    ...
    for (i = 0; i < argc; i++) {
        if (!JS_IsString(argv[i])) {
            JS_ThrowTypeError(ctx, "dynajs:path: \"path\" argument must be a string");
            failed = 1;
            break;
        }
                                  ...
                                  int argc, JSValueConst *argv)
{
    ...
    if (argc < 1 || !JS_IsString(argv[0]))
        return JS_ThrowTypeError(ctx, "dynajs:path: \"path\" argument must be a string");
                                 ...
                                 int argc, JSValueConst *argv)
{
    ...
    if (has_suffix && !JS_IsString(argv[1]))
        return JS_ThrowTypeError(ctx, "dynajs:path: \"suffix\" argument must be a string");
                                 ...
                                 int argc, JSValueConst *argv)
{
    ...
    if (argc < 1 || !JS_IsString(argv[0]))
        return JS_ThrowTypeError(ctx, "dynajs:path: \"from\" argument must be a string");
    if (argc < 2 || !JS_IsString(argv[1]))
        return JS_ThrowTypeError(ctx, "dynajs:path: \"to\" argument must be a string");
...
{
    JSModuleDef *m = JS_NewCModule(ctx, "dynajs:path", dyn_path_init_module);
    if (!m)
        return -1;
