/*
 * dyna:sys -- unified system interface: filesystem metadata + directories +
 ...
 * across os/path/filepath/io/fs, dyna:sys unifies the filesystem+process+
 * environment surface into ONE module with clean names. Path-STRING logic
 * (join/normalize/dirname/...) stays in dyna:path; buffered file CONTENT I/O
 * (FileReader/FileWriter/readFile/writeFile) stays in dyna:file -- this
