/*
 * dyna:encoding -- binary/text encodings, mirroring Go's encoding/hex,
 ...
 *            base85Encode, base85Decode } from "dyna:encoding";
 *
 ...
 * by dyna:bytes, extended with dyna:text's string fallback since the
 ...
 * base64_encode/base64_decode (dyna-simd-kernels.h, the same engine
 * dyna:search/dyna:text/dyna:bytes run on) for the standard alphabet;
 ...
 * extended-hex alphabets (JS_CFUNC_MAGIC_DEF, matching dyna:bytes' width/
 ...
 * modulo 2^64, matching dyna:bytes' writeBigUint64LE/writeBigInt64LE
 * convention).
...
#include "dyna-simd-kernels.h" /* base64_encode/base64_decode, shared with text/bytes */
