/*
 * dyna:text -- SIMD-accelerated byte/text utilities over the shared kernel
 * engine (dyna-simd-kernels.h). Self-contained, in-repo (no external deps).
 *
 ...
 * JS_ToCStringLen (offsets/counts are byte-based, matching dyna:search).
...
int js_nat_init_text(JSContext *ctx)
{
    ...
    m = JS_NewCModule(ctx, "dyna:text", dyn_text_init_module);
    if (!m)
        return -1;
