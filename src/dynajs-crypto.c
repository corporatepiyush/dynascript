/*
 * dynajs:crypto -- cryptographic hashes, HMAC and CRC. Self-contained, in-repo
 ...
 *
 *   import { sha256, sha256Hex, hmacHex, crc32, Hasher } from "dynajs:crypto";
 *
 ...
 * The streaming Hasher is a dynajs-nat resource (malloc-backed, freed by
...
/* ==================================================================== *
 *  streaming Hasher (dynajs-nat resource)                               *
 * ==================================================================== */
...
{
    JSModuleDef *m = JS_NewCModule(ctx, "dynajs:crypto", dyn_crypto_init_module);
    if (!m)
        return -1;
