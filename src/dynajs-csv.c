/*
 * dynajs:csv -- file-oriented CSV CRUD with SIMD-accelerated parsing and
...
/* Atomic durable write: temp file in the same dir, fsync, rename over `path`. */
...
int js_nat_init_csv(JSContext *ctx) {
    ...
    JSModuleDef *m = JS_NewCModule(ctx, "dynajs:csv", csv_module_init);
    if (!m) return -1;
