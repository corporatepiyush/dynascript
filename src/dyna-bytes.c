/*
 * dyna:bytes -- byte-buffer utilities over Uint8Array/ArrayBuffer, mirroring
 ...
 *
 *   import { compare, indexOf, readUint32LE, writeUint32LE, toHex } from "dyna:bytes";
 ...
 * search reuses the shared SIMD substring kernel (dyna-simd-kernels.h,
 * the same engine dyna:search/dyna:text run on).
 *
...
#include "dyna-simd-kernels.h" /* strfind / base64 kernels, shared with search + text */
 ...
 * semantics), unlike dyna:search's indexOfAll (which is deliberately
 ...
 * this is NOT validated UTF-8; use dyna:text's isValidUtf8 first if that
