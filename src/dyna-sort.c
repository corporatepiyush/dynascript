/*
 * dyna:sort -- native sorting + binary search. Self-contained, in-repo (no
 * external deps).
 *
...
static int dyn_sort_arr_len(JSContext *ctx, JSValueConst v, uint32_t *out_len)
{
    ...
    if (isarr < 0)
        return -1;
    if (!isarr) {
        JS_ThrowTypeError(ctx, "dyna:sort: expected an Array");
        return -1;
    }
                            ...
                            int idx, JSValueConst *pcmp)
{
    ...
    if (!JS_IsFunction(ctx, argv[idx])) {
        JS_ThrowTypeError(ctx, "dyna:sort: comparator is not a function");
        return -1;
    }
    *pcmp = argv[idx];
    return 0;
                                     ...
                                     uint32_t len)
{
    ...
    if (bytes / sizeof(JSValue) != len) { /* size_t multiply overflow */
        JS_ThrowRangeError(ctx, "dyna:sort: array too large");
        return NULL;
    }
...
{
    JSModuleDef *m = JS_NewCModule(ctx, "dyna:sort", dyn_sort_init_module);
    if (!m)
        return -1;
