...
/* ---------- OS entropy (same mechanism as src/dynajs-random.c) ---------- */
 ...
 * plain ArrayBuffer -> raw pointer + length), same shape as dynajs:encoding's
                           ...
                           int argc, JSValueConst *argv)
{
    ...
    if (clock_gettime(CLOCK_REALTIME, &ts) != 0)
        return JS_ThrowInternalError(ctx, "dynajs:uuid: clock_gettime failed");
...
{
    JSModuleDef *m = JS_NewCModule(ctx, "dynajs:uuid", dyn_uuid_init_module);
    if (!m)
        return -1;
